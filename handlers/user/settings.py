...
async def edit_lang_command(call: CallbackQuery, translator: LocalizedTranslator, request:Request):
    source_lang = (await request.get_user(call.from_user.id)).lang
    await request.change_user_lang(user_id=call.from_user.id, lang=call.data.split('_')[0])
    await call.message.edit_text(text=translator.get('confirmLang').split(', ')[0], reply_markup=await confirm(key='lang', translator=translator, lang=source_lang))
...
@settings.callback_query(F.data.startswith('back_lang'))
async def back_lang(call: CallbackQuery, translator: LocalizedTranslator, request:Request):
    data = call.data.split('_')
    await request.change_user_lang(user_id=call.from_user.id, lang=data[2])
    await call.message.edit_text(text=translator.get('welcome'), reply_markup=await menu(translator))
